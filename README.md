...
This is the access request generated by PEP in the ABAC architecture. It is created using the `Request` object in py-ABAC. All you need to do is take any kind of incoming user request (REST request, SOAP, etc.) and build a `Request` object out of it in order to feed it to py-ABAC. 
...
This object represents the PIP in the access control architecture. The purpose of this object is to provide attribute values which are missing from `Request` object. During policy evaluation, the `PDP` first checks the `Request` object for attribute values; If no values are found, it then checks the list of `AttributeProvider`objects used during its creation. In order to create an `AttributeProvider` object, you just need to inherit the `AttributeProvider` interface, and implement the `get_attribute_value` method. 
    ...
    def get_attribute_value(self, ace, attribute_path, ctx):
        """
            ...
            :param attribute_path: string in ObjectPat notation indicating 
                                   path and attribute for which the value 
                                   is requested.
...
As can be seen in the above example, the `get_attribute_value` method takes in three arguments, `ace`, `attribute_path` and `ctx`. The `ace` is a string value indicating for which access control element the attribute value is requested. This argument will be set to either `"subject"`, `"resource"`, `"action"`, or `"context"`. 
